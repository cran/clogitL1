...
#define LAMBDA_MIN_RATIO 0.00000001
...
static void getLinearPredictors(const vector<int>& y, const vector<double>& X, int K, double *beta, int nonZeroBeta, vector<int>& nonZeroBetaInd, int p, const vector<int>& nVec, double **linPred, double *maxLinPred, double *sumLinPredCases, vector<int>& keep){
	/*
	...
	*/
	int sumN = 0;
	for (int k = 0; k < K; k++){
		getLinearPredictorsInStratum(y, X, k, beta, nonZeroBeta, nonZeroBetaInd, p, sumN, nVec[k], linPred[k], &(maxLinPred[k]), &(sumLinPredCases[k])); // only compute the linear predictors should for non left out strata
		sumN += nVec[k];
	}
}
...
static void getLikelihood (vector<int>& whichVars, int numVars, int K, const vector<int>& nVec, const vector<int>& mVec, double **linPred, double *maxLinPred, double *sumLinPredCases, double *loglik, vector<int>& keep){
	/*
	...
	*/
	...
	//*loglik = 0;
	for (int k = 0; k < K; k++){
		if (keep[k] == 1) tempSum += getLikelihoodInStratum(whichVars, numVars, nVec[k], mVec[k], linPred[k], maxLinPred[k], sumLinPredCases[k]);
	}
	*loglik = tempSum;
}

static void getLikelihoodAndScoreInStratum (vector<int>& whichVars, int numVars, const vector<double>& X, int p, int stratumPos, int n, int m, double *linPred, double maxLinPred, double sumLinPredCases, const double *xMean, double *caseSums, double *loglik, double *score, int first){
	/*
	*	Computes the likelihood and score for 'whichVars' in stratum k
	*/
...
static void getLikelihoodAndScore (vector<int>& whichVars, int numVars, const vector<double>& X, int p, int K, const vector<int>& nVec, const vector<int>& mVec, double **linPred, double *maxLinPred, double *sumLinPredCases, double **xMean, double **caseSums, double *loglik, double *score, vector<int>& keep){
	/*
	*	Computes the likelihood and score (need to recursive calculations from the former for the latter) for all the variables the indices of which are listed in 'whichVars' (there are 'numVars' of them)
	* 	Arguments X, K, nVec, mVec, linPred, maxLinPred, sumLinPredCases, xMean and caseSums are all pre computed and ready for use in the recursive computation
	* 	The results will be stored in 'loglik' and 'score'
	*/
	...
	for (int k = 0; k < K; k++){
		if (keep[k] == 1){
			getLikelihoodAndScoreInStratum(whichVars, numVars, X, p, sumN, nVec[k], mVec[k], linPred[k], maxLinPred[k], sumLinPredCases[k], xMean[k], caseSums[k], loglik, score, first);
			first = 0;
		}	
...
static void getLikelihoodScoreAndHessianInStratum (vector<int>& whichVars, int numVars, const vector<double>& X, int p, int stratumPos, int n, int m, double *linPred, double maxLinPred, double sumLinPredCases, double *xMean, double *caseSums, double *loglik, double *score, double **hessian, int first){
	/*
	*	Computes the likelihood and score for 'whichVars' in stratum k
	*/
	double *normConst = (double*) calloc (m + 1, sizeof(double)); // will store the results of the recursive computations building up to the value of the normalising constant
	...
	*loglik += (sumLinPredCases - m*maxLinPred - log(normConst[m]));
	for (int s = 0; s < numVars; s++){
		...
		if (first == 1) score[whichVars[s]] = 0;
		...
		for (int t = s; t < numVars; t++){
			...
			double *normConstDeriv_t = normConstDeriv + t*(m+1);
			double *normConstDeriv2_st = normConstDeriv2 + (m+1)*(s + numVars*t);
			if (first == 1) hessian [minVar][abs(current_s - current_t)] = 0;
...
static void getLikelihoodScoreAndHessian (vector<int>& whichVars, int numVars, const vector<double>& X, int p, int K, const vector<int>& nVec, const vector<int>& mVec, double **linPred, double *maxLinPred, double *sumLinPredCases, double **xMean, double **caseSums, double *loglik, double *score, double **hessian, vector<int>& keep){
	/*
	...
	*/
	...
	for (int k = 0; k < K; k++){
		if (keep[k] == 1){
			getLikelihoodScoreAndHessianInStratum(whichVars, numVars, X, p, sumN, nVec[k], mVec[k], linPred[k], maxLinPred[k], sumLinPredCases[k], xMean[k], caseSums[k], loglik, score, hessian, first);
			first = 0;
		}
		sumN += nVec[k]; // keep track of which row we are at in the X matrix
	}
...
NumericMatrix fit_cloglik (const vector<int>& y, const vector<double>& X, int K, int p, const vector<int>& mVec, const vector<int>& nVec, vector<double>& lambdaVec, int numLambda, vector<int>& keep, double alpha){
	/*
	*	Function for fitting the conditional loglikelihood using coordinate descent (with some strata left out)
	*		input:
	...
	*			- lambdaVec = Vector containing the lambda values of interest
	*			- numLambda = Number of lambdas
	*			- keep = Vector with 1 for stratum to keep; 0 to leave out
	*/
	
	int firstMiddle, count, countUp, numViolations, nonZeroBeta;
	...
	getLikelihoodAndScore (notStrongSet, p-numStrongSet, X, p, K, nVec, mVec, linPred, maxLinPred, sumLinPredCases, xMean, caseSums, &loglik, score, keep);
	penalty = 0;
	...
	for (int l = 1; l <= numLambda; l++){ // loop over lambdas
		...
		while (numViolations > 0){
			...
			while (firstMiddle || fabs((-loglik+oldloglik+penalty-oldpenalty)/(-loglik + penalty)) > eps){ // quadratic approximation to the loglikelihood - middle loop
				...
				oldloglik = loglik;
				oldpenalty = penalty;
				getLikelihoodScoreAndHessian (strongSet, numStrongSet, X, p, K, nVec, mVec, linPred, maxLinPred, sumLinPredCases, xMean, caseSums, &loglik, score, hessian, keep);
				...
				getLinearPredictors (y, X, K, currentBeta, numStrongSet, strongSet, p, nVec, linPred, maxLinPred, sumLinPredCases, keep);
				getLikelihood (strongSet, numStrongSet, K, nVec, mVec, linPred, maxLinPred, sumLinPredCases, &loglik, keep);
			...
			if (count != 100){
				getLikelihoodAndScore (strongSet, numStrongSet, X, p, K, nVec, mVec, linPred, maxLinPred, sumLinPredCases, xMean, caseSums, &loglik, score, keep);
				getLikelihoodAndScore (notStrongSet, p-numStrongSet, X, p, K, nVec, mVec, linPred, maxLinPred, sumLinPredCases, xMean, caseSums, &loglik, score, keep); // get the score of all the other variables
				numViolations = 0;
				for (int i = 0; i < p-numStrongSet; i++){ // loop over predictors that are not in the strong set
					if (fabs(score[notStrongSet[i]] - lambdaVec[l]*(1-alpha)*currentBeta[notStrongSet[i]]) > lambdaVec[l]) {
						...
						for (int j = i; j < p-numStrongSet;j++){
		...
		beta (l, p+1) = nonZeroBeta;
		beta (l, p+2) = numStrongSet;
		beta (l, p+3) = count;
		beta (l, p+4) = countUp;
		
		// compute the CV error estimate for this fold
		vector<int> leaveOut(K);
		for (int k = 0; k < K; k++){
			leaveOut[k] = 1-keep[k];
		}
		double cvErr = 0;
		getLikelihood (strongSet, numStrongSet, K, nVec, mVec, linPred, maxLinPred, sumLinPredCases, &cvErr, leaveOut);
		beta (l, p+5) = cvErr;
		if (-2*loglik - nullDev >= 0.99*nullDev) break; // for wide datasets - see Simon et al (2011)
	}
	//printf ("\nOut of for loop\n");
...
static void copyData (NumericVector yvec, NumericMatrix xmat, vector<int>& y, vector<double>& X, NumericVector lambdas, vector<double>& lambdaVec, NumericVector keepvec, vector<int>& keep){
	/*
	*	Copies elements of 'yvec' into 'y' and elements of 'xmat' into 'X'
	*/
	int numKeep = keepvec.size();
	for (int l = 0; l < numKeep; l++){
		keep[l] = (int)keepvec[l];
	}
	int numLambdas = lambdas.size();
	for (int k = 0; k < numLambdas; k++){
		lambdaVec[k] = lambdas[k];
	}
...
NumericMatrix clogitl1CV_c (NumericVector n, NumericVector m, NumericMatrix Xmat, NumericVector yvec, NumericVector lambdas, NumericVector keepvec, double alpha){
	/*
	*	Function similar to clogitl1 
	* 	Now we supply 'lambdas' - grid of lambdas already determined (to ensure correspondence over different CV folds)
	*	Also supply 'keepvec' - a vector of length K with 1 at the index of a stratum we keep for this iteration and 0 for one we leave out
	*/
	...
	vector<double> lambdaVec(lambdas.size());
	vector<int> keep(K);
	copyData(yvec, Xmat, y, X, lambdas, lambdaVec, keepvec, keep);
	//printf ("Data copied\n");
	
	// fit the model
	NumericMatrix beta = fit_cloglik(y, X, K, p, mVec, nVec, lambdaVec, lambdas.size()-1, keep, alpha);
